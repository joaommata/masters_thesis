...
from c2_prepare_data_simulated_cf import compute_cf_for_split
...
def run_cv(disease, cf_count, save_fold_data=True):
    """
    ...
    cv_dir = os.path.join(results_base, f'C2_sim_cf/{disease}/cv_results/cf_{cf_count}')
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(full_df, y)):
        ...
        fold_train_cf, fold_test_cf, _ = compute_cf_for_split(
            train_df=fold_train,
            test_df=fold_test,
            cf_count=cf_count,
            disease=disease,
        ...
        # Optionally save fold data to disk
        if save_fold_data:
