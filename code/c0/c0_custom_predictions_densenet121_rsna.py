"""
c0_custom_predictions_densenet121_rsna.py
Runs DenseNet121 inference + Grad-CAM + embeddings + SDN internal classifiers
for the RSNA Pneumonia binary task (pneumonia vs everything else).

Same structure as c0_custom_predictions_densenet121.py -- the differences are:
  - DICOM loading (rescaled to 8-bit) instead of PIL/JPEG
  - target column is 'Pneumonia' (no per-disease column map)
  - Grad-CAM filenames derive from the .dcm path
  - the split CSVs carry a 'view' column, which is passed through to the output
    so the acquisition confound stays analysable downstream

Only the VAL split is scored for C2 -- see c1_build_attribute_vector_rsna.py.
The train split is still run because the SDN internal-classifier heads are
trained on it (backbone frozen), matching the CheXpert script.
"""
...
OUTPUT_DIR = os.path.join(RESULTS_DIR, "C0_custom/rsna_pneumonia")
...
banner("CONFIG")
print(f"Task:           RSNA pneumonia vs rest (column: {TARGET_COL})")
...
class RSNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Must match the training-time preprocessing in
        # c0_train_densenet121_rsna.py exactly: DICOM -> per-image 8-bit rescale
        # -> RGB -> ImageNet normalisation.
        arr = pydicom.dcmread(os.path.join(self.data_dir, path)).pixel_array
...
def train_internal_classifiers(model, ics, df):
    ...
    print(f"Loss:             BCEWithLogits vs. ground-truth "
          f"'{TARGET_COL}' label (same target as final layer)")
    print("Backbone runs under torch.no_grad() — its weights cannot change.")

    loader = DataLoader(
        RSNADataset(df, DATA_DIR, transform),
...
def run_inference(model, ics, loader, cam_dir, split=""):
    ...
    for imgs, labels, paths in tqdm(loader, desc=f"inference {split}"):
        ...
        for i, path in enumerate(paths):
            ...
            # RSNA paths are StudyUID/SeriesUID/SOPUID.dcm -- flatten to a single
            # filename the same way the CheXpert script flattens its .png paths.
            cam_path = os.path.join(
                cam_dir,
                path.replace("/", "_").replace(".dcm", "_cam.npz")
            )
...
def main():
    ...
    train_loader = DataLoader(
        RSNADataset(train_df, DATA_DIR, transform),
        batch_size=BATCH_SIZE,
        shuffle=False,
        num_workers=args.workers,
    )
    ...
    val_loader = DataLoader(
        RSNADataset(val_df, DATA_DIR, transform),
        batch_size=BATCH_SIZE,
        shuffle=False,
        num_workers=args.workers,
    )
    ...
    banner("METRICS")
    # Threshold is picked on TRAIN (Youden's J) and applied to val, so the val
    # correctness labels never see their own optimal cut.
    ...
    # Carry the view + label_name columns through so the acquisition confound and
    # the hard-negative subtype stay analysable at C2 time.
    for out, src in [(train_df_out, train_df), (val_df_out, val_df)]:
        meta = src[["Path", "view", "label_name"]].rename(columns={"Path": "path"})
        out_merged = out.merge(meta, on="path", how="left")
        out[["view", "label_name"]] = out_merged[["view", "label_name"]]

    banner("SAVING OUTPUTS")
    train_csv = os.path.join(OUTPUT_DIR, "train_c0_rsna_pneumonia.csv")
    val_csv   = os.path.join(OUTPUT_DIR, "val_c0_rsna_pneumonia.csv")
    ...
    # Threshold is needed by downstream CF/flip analyses (see threshold.txt
    # convention in the CheXpert pipeline).
    with open(os.path.join(OUTPUT_DIR, "threshold.txt"), "w") as f:
        f.write(str(thresh))
    print(f"Wrote threshold.txt ({thresh:.4f})")
