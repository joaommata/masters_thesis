"""
c2_cv_pipeline_new_split_entropy.py
...
Configs:
    ...
    Multi-modal (M1–M6):
        ...
        M2 : disease probability entropy + delta features
        M3 : disease probability entropy + delta + attribute features
        M4 : disease probability entropy + delta + embedding features
        M5 : disease probability entropy + delta + attribute + embedding features
        M6 : disease probability entropy + delta + attribute + CF entropy

    CF-enriched (MCF1–MCF5) — unmatched path only, progressive accumulation:
        MCF2 : disease probability entropy + attr + cf_attr
        MCF3 : disease probability entropy + attr + cf_attr + ΔA
        MCF4 : disease probability entropy + attr + cf_attr + ΔA + emb + cf_emb
        MCF5 : disease probability entropy + attr + cf_attr + ΔA + emb + cf_emb + Δemb
...
def build_feature_matrices(train_df, test_df, disease):
    ...
    entropy_tr = -prob_tr * np.log(prob_tr + 1e-10) - (1 - prob_tr) * np.log(1 - prob_tr + 1e-10)
    entropy_te = -prob_te * np.log(prob_te + 1e-10) - (1 - prob_te) * np.log(1 - prob_te + 1e-10)
    ...
    configs = {
        ...
        'M1': (diff_tr, diff_te),
        'M2': (np.hstack([entropy_tr, diff_tr]),              np.hstack([entropy_te, diff_te])),
        'M3': (np.hstack([entropy_tr, attr_tr]),     np.hstack([entropy_te, attr_te])),
        'M4': (np.hstack([entropy_tr, emb_tr]),      np.hstack([entropy_te, emb_te])),
        'M5': (np.hstack([entropy_tr, attr_tr, emb_tr]), np.hstack([entropy_te, attr_te, emb_te])),
        'M6': (np.hstack([entropy_tr, attr_tr, cf_prob_tr]), np.hstack([entropy_te, attr_te, cf_prob_te])),
    }
    ...
    if cf_attr_cols and cf_emb_cols:
        ...
        configs.update({
            'MCF1': (np.hstack([entropy_tr, cf_prob_tr]),
                     np.hstack([entropy_te, cf_prob_te])),
            'MCF2': (np.hstack([entropy_tr, cf_prob_tr, attr_tr, cf_attr_tr]),
                     np.hstack([entropy_te, cf_prob_te, attr_te, cf_attr_te])),
            'MCF3': (np.hstack([entropy_tr, cf_prob_tr, attr_tr, cf_attr_tr, diff_tr]),
                     np.hstack([entropy_te, cf_prob_te, attr_te, cf_attr_te, diff_te])),
            'MCF4': (np.hstack([entropy_tr, cf_prob_tr, attr_tr, cf_attr_tr, diff_tr, emb_tr, cf_emb_tr]),
                     np.hstack([entropy_te, cf_prob_te, attr_te, cf_attr_te, diff_te, emb_te, cf_emb_te])),
            'MCF5': (np.hstack([entropy_tr, cf_prob_tr, attr_tr, cf_attr_tr, diff_tr, emb_tr, cf_emb_tr, delta_emb_tr]),
                     np.hstack([entropy_te, cf_prob_te, attr_te, cf_attr_te, diff_te, emb_te, cf_emb_te, delta_emb_te])),
        })
