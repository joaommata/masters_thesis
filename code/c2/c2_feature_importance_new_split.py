...
# ----- PLOTTING -------
# ======================
def plot_group_importance(df, title, color_map=None, save_path=None, metric='sum'):
...
def main(disease, config, model_type, cf_count):
    cache_dir = os.path.join(RESULTS_DIR, 'C2_importances_cache/new_split')
    all_preds = []
    print("Starting feature importance computation...")
    for fold in range(N_FOLDS):
        print(f"Processing fold {fold+1}/{N_FOLDS}...")
        fold_path = os.path.join(RESULTS_DIR,
                                 f'C2_custom/{disease}/cv_results/cf_{cf_count}/fold_{fold}_predictions.csv')
