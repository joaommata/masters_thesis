...
Configs:
    ...
    Multi-modal (M1–M6):
        ...
        ---
        ADDED AFTER THE GREAT DISASTER - TRYING TO PUSH AUC UP BY ADDING MORE CF INFO TO THE MODELS.
        ---
        
        M7:  disease probability + delta + attribute + + embedding features + CF probability 
        M8:  disease probability + delta + attribute + embedding features + CF probability + CF attributes
        M9:  disease probability + delta + attribute + CF probability + embedding features + CF attributes + CF embeddings
...
from c2_prepare_data_simulated_cf import compute_cf_for_split, compute_cf_for_split_unmatched, compute_cf_for_split_further, compute_cf_for_split_matched_train_unmatched_test # added the new one for the "further" cfs on the disease absent
...
#CONFIGS = ['M6']  # For quick testing, comment out to run all configs
...
def build_feature_matrices(train_df, test_df, disease):
    ...
    # Get the features from both train and test
    ...
    # Return a dictionary with the selected features for each type of config
    return {
        'B1': (prob_tr, prob_te),
        'B2': (attr_tr, attr_te),
        'B3': (emb_tr,  emb_te),
...
def run_cv(disease, cf_count, save_fold_data=False, unmatched=False, backbone='densenet', k_offset=0):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(full_df, y)):
        ...
        if k_offset > 0:
            ...
            cf_fn = compute_cf_for_split_matched_train_unmatched_test
            fold_train_cf, fold_test_cf, _ = cf_fn(
                train_df=fold_train, test_df=fold_test,
                cf_count=cf_count, disease=disease, distance='l1'
            )
