...
SPLIT_DIR  = os.path.join(BASE_DIR, "data/effusion")
OUTPUT_DIR = os.path.join(BASE_DIR, "results/C0_vit/effusion")
MODEL_PATH = os.path.join(BASE_DIR, "results/C0_vit/c0_best.pt")
BATCH_SIZE = 16
...
def run_inference(model, loader, embeddings_cache, device):
    ...
    for imgs, labels, paths in tqdm(loader):
        ...
        emb_batch = embeddings_cache['last']  # (B, 768)
...
def main():
    ...
    # Hook on the final encoder block — captures CLS token, output (B, 768)
    embeddings_cache = {}
    def hook_fn(module, input, output):
        # output is (B, seq_len, 768) — take CLS token at position 0
        embeddings_cache['last'] = output[:, 0, :].cpu().numpy()
    model.encoder.layers[-1].register_forward_hook(hook_fn)
