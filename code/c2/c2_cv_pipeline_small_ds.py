...
def run_cv(disease, cf_count, save_fold_data=True, unmatched=False, subsample_size=None):
    ...
    cv_dir = os.path.join(results_base, f'C2_custom/{disease}/{cv_subdir}/cf_{cf_count}{subsample_tag}')
    ...
    # ── Load dataset ──
    full_df = pd.read_csv(os.path.join(results_base, '/work3/s251710/thesis_results/C2_custom/effusion/c2_data.csv'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_save_folds', action='store_true', default=True, help='Skip saving individual fold CSVs')
