...
BASE_DIR    = "/zhome/d0/a/221493/thesis"  # code/checkpoints only (read once)
C0_CSV         = f"{RESULTS_DIR}/C0_custom/effusion/val_c0_effusion.csv"
C0_MODEL_PATH  = f"{RESULTS_DIR}/C0_custom/effusion/c0_best.pt"
C2_CSV = f"{RESULTS_DIR}/C2_custom/c2_data.csv"
FOLD_CSV = f"{RESULTS_DIR}/C2_custom/effusion/cv_results/cf_1/fold_0_predictions.csv"
...
OUTPUT_PATH    = f"{RESULTS_DIR}/diffusion_cf/{T_START}/{GUIDANCE_WEIGHT}"
...
def one_guided_step(unet, sd, x_t, t_val, classifier, target_class, guidance_weight=GUIDANCE_WEIGHT):
    """
...
def generate_cf(unet, sd, x0, classifier, target_class, n_snapshots=N_SNAPSHOTS, track_probs=True,
                t_start=T_START, guidance_weight=GUIDANCE_WEIGHT):
    """
    ...
    # t_start controls how noisy the initial image is: higher means more freedom but less identity preservation.
    ...
    # Reverse loop from t_start down to 1
    for t_val in tqdm(reversed(range(1, t_start + 1)), total=t_start, desc="Guided reverse"):
        x = one_guided_step(unet, sd, x, t_val, classifier, target_class, guidance_weight=guidance_weight)
