"""
c2_compare_selection_strategies.py
===================================
...
Strategies compared (subdirs under results/C2_custom_corrected/{disease}/):
  ...
  gt_routing           - oracle: chooses the ground truth copposite of the prediction
...
Usage:
    python c2_compare_selection_strategies.py --disease effusion --cf_count 1
"""
...
def load_strategy_data(disease, cf_count, models):
    ...
    input_root = os.path.join(RESULTS_DIR, '', 'C2_custom_corrected', disease)
...
def plot_strategy_comparison(long_df, model_type, output_dir, cf_count, disease):
    ...
    ax.set_title(f'CF Selection Strategy Comparison — {disease.capitalize()} {model_type} (CF={cf_count})')
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  CF SELECTION STRATEGY COMPARISON: {args.disease.upper()} — CF={args.cf_count}")
    print(f"{'='*70}\n")

    long_df = load_strategy_data(args.disease, args.cf_count, args.models)

    output_dir = os.path.join(RESULTS_DIR, '', 'C2_custom_corrected', args.disease,
