"""
cv_analyse_results_new_split.py
=====================
...
def plot_roc_curves(cv_results, disease, cf_count, output_dir):
    ...
    for ax, model_type in zip(axes, ['LR', 'RF', 'MLP']):
        ...
        if C2_MODEL == 'C2_custom':
            C2_MODEL_LABEL = 'C2_Densenet121'
        else:
            C2_MODEL_LABEL = C2_MODEL
        ax.set_title(f'{model_type} | {C2_MODEL} | {disease.capitalize()} \n (CF={cf_count})')
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
...
def main():
    ...
    # Output directory
    output_base = os.path.join(RESULTS_DIR, f'{C2_MODEL}/{disease}/cv_analysis')
    ...
    for cf_count in cf_counts:
        cv_dir = os.path.join(RESULTS_DIR, f'{C2_MODEL}/{disease}/cv_results/cf_{cf_count}')
