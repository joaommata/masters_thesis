...
c2_cnn_all.py
=============
Unified CNN baselines for C2 quality control under the "correct CF" rule.

Trains models in sequence using the same CV fold splits:
  1. CNN Xi         — query image xi only
  2. CNN Xi + Saliency Maps — query image + C0 Grad-CAM of xi as a second encoder stream
  3. CNN CF         — counterfactual image only
  4. CNN CF + Saliency Maps — CF image + C0 Grad-CAM of CF as a second encoder stream
  5. Dual Encoder   — query + CF via two separate DenseNet encoders
  6. Dual Encoder + Saliency Maps — query + CF + their C0 Grad-CAMs via four separate encoders
  7. Dual Encoder + Saliency + Scalars — config 6 plus a per-CF vector of C0
...
Use --models to train only a subset (default: all six), e.g.:
    ...
    python c2_cnn_all.py --models xi_sal cf_sal dual_sal
...
paths (K = 1, 3, 5, 16, ...), every CF-facing dataset loads all K images (and
their Grad-CAMs). The model is run once per counterfactual to get K
predictions, which are combined into one final prediction by averaging their
probabilities — an ensemble over the K counterfactuals, mirroring how the
tabular pipeline averages predictions over the K nearest neighbours. Pass
--cf_count to select which fold set (cf_{K}) to train on; one process handles
a single K.
...
from sklearn.model_selection import GroupShuffleSplit   # was: train_test_split
...
def _nearest_cf_only(df):
    ...
    sample contributes ONE pair per epoch. cf_paths lists are nearest-first
...
DualDataset = QueryCFDataset


class QueryDatasetWithSaliency(Dataset):
    """Query image xi + C0 Grad-CAM of xi as a second encoder stream."""
    def __init__(self, df, data_dir, transform=None):
        self.df, self.data_dir, self.transform = df.reset_index(drop=True), data_dir, transform
...
class CFDatasetWithSaliency(Dataset):
    """K CF images + their C0 Grad-CAMs, each stacked as (K, 3, 224, 224)."""
    def __init__(self, df, data_dir, transform=None):
        self.df, self.data_dir, self.transform = df.reset_index(drop=True), data_dir, transform
    ...
    def __getitem__(self, idx):
        ...
        for cf_path in _split_cf_paths(row['cf_paths']):
            ...
            if self.transform:
                img = self.transform(img)
            imgs.append(img)
            cams.append(_load_cam(cf_path))
        return torch.stack(imgs), torch.stack(cams), float(row['correct'])


class QueryCFDatasetWithSaliency(Dataset):
    """Query + K CF images + their respective C0 Grad-CAMs (four encoder streams,
    the CF-side two stacked as (K, 3, 224, 224))."""
    def __init__(self, df, data_dir, transform=None):
        self.df, self.data_dir, self.transform = df.reset_index(drop=True), data_dir, transform
    ...
    def __getitem__(self, idx):
        ...
        for cf_path in _split_cf_paths(row['cf_paths']):
            ...
            if self.transform:
                cf_img = self.transform(cf_img)
            cf_imgs.append(cf_img)
            cf_cams.append(_load_cam(cf_path))

        return q_img, torch.stack(cf_imgs), xi_cam, torch.stack(cf_cams), float(row['correct'])


class QueryCFDatasetWithSaliencyScalars(Dataset):
    """QueryCFDatasetWithSaliency plus the C0 prediction scalars as a (K, 5)
    tensor inserted before the label."""
    def __init__(self, df, data_dir, transform=None):
        self.df, self.data_dir, self.transform = df.reset_index(drop=True), data_dir, transform
    ...
    def __getitem__(self, idx):
        ...
        return (q_img, torch.stack(cf_imgs), xi_cam, torch.stack(cf_cams),
...
# combined into one prediction by the ensembling helpers further down —
# the models themselves have no notion of K.
...
class DualEncoderCNN(nn.Module):
    ...
    def forward(self, query_img, cf_img):
        emb = torch.cat([self.query_enc(query_img), self.cf_enc(cf_img)], dim=1)
        return self.classifier(emb)


class QuadEncoderCNN(nn.Module):
    """Four images (query, CF, query CAM, CF CAM), each with its own encoder,
    ...
    def forward(self, query, cf, qcam, cfcam):
        emb = torch.cat([
            self.query_enc(query), self.cf_enc(cf),
...
class QuadEncoderScalarCNN(nn.Module):
    """QuadEncoderCNN whose head also sees the C0 prediction scalars: the
    (B, 5) vector is embedded to 32 dims and concatenated with the four
    ...
    def __init__(self, hidden_dim=256, dropout=0.3, scalar_dim=5, scalar_hidden=32):
        ...
        self.scalar_embed = nn.Sequential(nn.Linear(scalar_dim, scalar_hidden),
                                          nn.ReLU(inplace=True))
        self.classifier = nn.Sequential(
            nn.Linear(1024 * 4 + scalar_hidden, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, 1),
        )

    def forward(self, query, cf, qcam, cfcam, scalars):
        emb = torch.cat([
            self.query_enc(query), self.cf_enc(cf),
            self.qcam_enc(qcam),   self.cfcam_enc(cfcam),
            self.scalar_embed(scalars),
        ], dim=1)
        return self.classifier(emb)
...
# prediction — a genuine ensemble over the K counterfactuals, matching how the
# tabular pipeline averages predictions over the K nearest neighbours.
...
def make_weighted_bce(pos_weight):
    ...
    reimplemented here because training now operates on ensembled
    probabilities rather than a single logit)."""
...
# ── Train / eval — quad input ──────────────────────────────────────────────────
def _train_quad(model, loader, optimizer, criterion, device, scaler):
    model.train()
    total = 0.0
    for q_imgs, cf_imgs, qcam_imgs, cfcam_imgs, labels in loader:
        streams = (q_imgs.to(device), cf_imgs.to(device),
                   qcam_imgs.to(device), cfcam_imgs.to(device))
        (q_imgs, cf_imgs, qcam_imgs, cfcam_imgs), b, k = _stack_cf_inputs(*streams)
        labels = labels.to(device)
        optimizer.zero_grad()
        with autocast():
            logits = model(q_imgs, cf_imgs, qcam_imgs, cfcam_imgs).squeeze(1)
        probs = _ensemble_probs(logits, b, k)
        loss  = criterion(probs, labels)
        scaler.scale(loss).backward()
        scaler.step(optimizer)
...
def _eval_quad(model, loader, device):
    model.eval()
    probs, labels = [], []
    with torch.no_grad(), autocast():
        for q_imgs, cf_imgs, qcam_imgs, cfcam_imgs, lbls in loader:
            streams = (q_imgs.to(device), cf_imgs.to(device),
                       qcam_imgs.to(device), cfcam_imgs.to(device))
            (q_imgs, cf_imgs, qcam_imgs, cfcam_imgs), b, k = _stack_cf_inputs(*streams)
            logits = model(q_imgs, cf_imgs, qcam_imgs, cfcam_imgs).squeeze(1).float()
            probs.extend(_ensemble_probs(logits, b, k).cpu().numpy())
            labels.extend(lbls.numpy())
    return roc_auc_score(labels, probs), probs, labels


# ── Train / eval — quad input + scalars ────────────────────────────────────────
...
def _train_quad_scalars(model, loader, optimizer, criterion, device, scaler):
    model.train()
    total = 0.0
    for q_imgs, cf_imgs, qcam_imgs, cfcam_imgs, scal, labels in loader:
        streams = (q_imgs.to(device), cf_imgs.to(device),
                   qcam_imgs.to(device), cfcam_imgs.to(device))
        (q_imgs, cf_imgs, qcam_imgs, cfcam_imgs), b, k = _stack_cf_inputs(*streams)
        ...
        optimizer.zero_grad()
        with autocast():
            logits = model(q_imgs, cf_imgs, qcam_imgs, cfcam_imgs, scal).squeeze(1)
        probs = _ensemble_probs(logits, b, k)
        loss  = criterion(probs, labels)
        scaler.scale(loss).backward()
        scaler.step(optimizer)
...
def _eval_quad_scalars(model, loader, device):
    model.eval()
    probs, labels = [], []
    with torch.no_grad(), autocast():
        for q_imgs, cf_imgs, qcam_imgs, cfcam_imgs, scal, lbls in loader:
            streams = (q_imgs.to(device), cf_imgs.to(device),
                       qcam_imgs.to(device), cfcam_imgs.to(device))
            (q_imgs, cf_imgs, qcam_imgs, cfcam_imgs), b, k = _stack_cf_inputs(*streams)
            scal   = scal.to(device).reshape(b * k, -1)
            logits = model(q_imgs, cf_imgs, qcam_imgs, cfcam_imgs, scal).squeeze(1).float()
            probs.extend(_ensemble_probs(logits, b, k).cpu().numpy())
            labels.extend(lbls.numpy())
    return roc_auc_score(labels, probs), probs, labels
...
def _embed_dual(model, loader, device):
    ...
    with torch.no_grad(), autocast():
        for a_imgs, b_imgs, lbls in loader:
            emb = torch.cat([
                _encode_pooled(model.query_enc, a_imgs.to(device)),
                _encode_pooled(model.cf_enc, b_imgs.to(device)),
            ], dim=1).float()
            embs.append(emb.cpu().numpy())
            labels.extend(lbls.numpy())
    return np.vstack(embs), np.array(labels)
...
def _embed_quad(model, loader, device):
    """Extract concatenated encoder embeddings (4096-dim) before the MLP head."""
    model.eval()
    embs, labels = [], []
    with torch.no_grad(), autocast():
        for q, cf, qcam, cfcam, lbls in loader:
            emb = torch.cat([
                _encode_pooled(model.query_enc, q.to(device)),
                _encode_pooled(model.cf_enc, cf.to(device)),
                _encode_pooled(model.qcam_enc, qcam.to(device)),
                _encode_pooled(model.cfcam_enc, cfcam.to(device)),
            ], dim=1).float()
            embs.append(emb.cpu().numpy())
            labels.extend(lbls.numpy())
    return np.vstack(embs), np.array(labels)
...
def run_cv(name, fold_dfs, dataset_cls, make_model, output_dir, mode='single'):
    """
    mode: 'single' | 'dual' | 'quad' | 'quad_scalars'
      single       — dataset yields (img, label)
      dual         — dataset yields (img_a, img_b, label)
      quad         — dataset yields (img_a, img_b, cam_a, cam_b, label)
      quad_scalars — dataset yields (img_a, img_b, cam_a, cam_b, scalars, label)
    """
    os.makedirs(output_dir, exist_ok=True)
    train_fn = {'single': _train_single, 'dual': _train_dual, 'quad': _train_quad,
                'quad_scalars': _train_quad_scalars}[mode]
    eval_fn  = {'single': _eval_single,  'dual': _eval_dual,  'quad': _eval_quad,
                'quad_scalars': _eval_quad_scalars}[mode]
...
if __name__ == '__main__':
    ...
    # Outer folds are inherited from the tabular CV pipeline's patient-grouped
    # split — verify that assumption instead of trusting it silently.
    for i in range(N_FOLDS):
        for j in range(i + 1, N_FOLDS):
            overlap = set(fold_dfs[i]['patient_id']) & set(fold_dfs[j]['patient_id'])
            assert len(overlap) == 0, (
                f"Patient overlap between outer folds {i} and {j}: {len(overlap)}. "
                "Regenerate fold_*_predictions.csv with the group-aware pipeline first."
            )
    print(f"Verified: zero patient overlap across {N_FOLDS} outer folds")
    ...
    # results/C2_cnn/{disease}/late/{config}/cf_{K}/ — "late" distinguishes
    # this script's outputs from c2_cnn_early_fusion.py's under the same
    # RES_BASE; config names match 1:1 across both scripts.
    out = lambda name: os.path.join(RES_BASE, DISEASE, 'late', name, f'cf_{CF_COUNT}')

    # Registry of all six configurations. Each entry is the kwarg set passed
    ...
    MODEL_REGISTRY = {
        ...
        'cf_sal': dict(
            name        = 'CNN CF + Saliency',
            dataset_cls = CFDatasetWithSaliency,
            make_model  = DualEncoderCNN,
        ...
        'dual_sal': dict(
            name        = 'Dual Encoder + Saliency',
            dataset_cls = QueryCFDatasetWithSaliency,
            make_model  = QuadEncoderCNN,
            output_dir  = out('dual_sal'),
            mode        = 'quad',
        ),
        'dual_sal_scalars': dict(
            name        = 'Dual Encoder + Saliency + Scalars',
            dataset_cls = QueryCFDatasetWithSaliencyScalars,
